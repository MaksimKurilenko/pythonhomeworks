...
class Bank():
    ...
    def transfer(self, sender, sender_currency, receiver, receiver_currency, amount):
        ...
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        sender_acc.history.append(f"[{timestamp}] Transfer to {receiver.name}: -{amount} {sender_currency}")
        receiver_acc.history.append(f"[{timestamp}] Transfer from {sender.name}: +{amount} {receiver_currency}")
